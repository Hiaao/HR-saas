...
export default {
  ...
  methods: {
    // 获取个人详情
    async getDetailById() {
      this.userInfo = await getDetailByIdAPI(this.userId)
    ...
    async saveUser() {
      ...
      if (fileList.some(obj => !obj.upload)) {
        // 还没有upload赋值，正在上传时
        this.$message.warning('文件正在上传，请稍后再试！')
        return
      }
      await saveUserDetailByidAPI({ ...this.userInfo, staffPhoto: fileList.length ? fileList[0].url : ' ' })
    ...
    async savePersonal() {
      ...
      await updatePersonalAPI({ ...this.formData, staffPhoto: fileList.length ? fileList[0].url : ' ' })
